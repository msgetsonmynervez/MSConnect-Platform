  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', gap: '8px' }}>
        {items.map((_, i) => {
          let bg = '#E0E0E0'
          if (i < current) bg = '#5C7A6B'
          if (i === current) bg = '#1C2B3A'
          const scale = i === current ? 'scale(1.2)' : 'scale(1)'
          return (
            <div key={i} style={{
              width: '10px',
              height: '10px',
    ...
    <div style={{ marginBottom: '24px' }}>
      ...
      <div style={{
        ...
        {vals.map(val => {
          ...
          const bg = selected ? '#1C2B3A' : '#F0EDE8'
          const color = selected ? '#FAF7F2' : '#6B7280'
          const fw = selected ? 600 : 400
...
function Confetti({ active }: { active: boolean }) {
  ...
  const colors = ['#5C7A6B', '#C4714A', '#D4A843', '#648FFF', '#8FAF9F']
  ...
  return (
    <div style={{
      ...
      {particles.map(p => {
        const anim2 = 'fall 1.4s ease-in ' + p.delay + 's forwards'
        return (
          <div key={p.id} style={{
            position: 'absolute',
            left: p.x + '%',
...
export default function DailyCheckIn() {
  ...
  useEffect(() => {
    async function init() {
      ...
      setCurrentStreak(streak ? streak.current_streak_days : 0)
      ...
      if (existing.data) {
        ...
        setForm({
          ...
          symptom_flags: e.symptom_flags ?? [],
          free_text_note: e.free_text_note ?? '',
          day_tag: e.day_tag ?? null,
        })
      } else if (prev.data) {
        const p = prev.data
        setForm(f => ({
  ...
  function toggleSymptom(symptom: string) {
    setForm(f => {
      const has = f.symptom_flags.includes(symptom)
      const next = has
        ? f.symptom_flags.filter(s => s !== symptom)
        : [...f.symptom_flags, symptom]
      return { ...f, symptom_flags: next }
    })
  }
  ...
  function startVoice() {
    ...
    r.onresult = (e: any) => {
      ...
      setForm(f => {
        const note = f.free_text_note
        return { ...f, free_text_note: note ? note + ' ' + t : t }
      })
    }
  ...
  const noteBorder = listening ? '#C4714A' : '#E0E0E0'
  const micBg = listening ? '#C4714A' : '#F0EDE8'
  const micIcon = listening ? '⏹️' : '🎤'
  const micTitle = listening ? 'Stop recording' : 'Record voice note'
          ...
          <div>
            ...
            <div style={{
              ...
              {SYMPTOMS.map(s => {
                ...
                const border = active ? '2px solid #1C2B3A' : '2px solid #E0E0E0'
                const bg = active ? '#1C2B3A' : '#FFFFFF'
                const color = active ? '#FAF7F2' : '#6B7280'
                return (
            ...
            <div style={{ display: 'flex', gap: '12px' }}>
              <button style={backStyle} onClick={() => setStep(0)}>
                Back
              </button>
              <button style={{ ...btnStyle, flex: 1 }} onClick={() => setStep(2)}>
                Continue →
              </button>
            </div>
          </div>
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: '12px', marginBottom: '24px' }}>
              {(['good', 'hard'] as const).map(tag => {
                const isSelected = form.day_tag === tag
                const activeColor = tag === 'good' ? '#5C7A6B' : '#3b82f6'
                const activeBg = tag === 'good' ? '#EDF3F0' : '#EFF6FF'
                const border = isSelected
                  ? '2px solid ' + activeColor
                  : '2px solid #E0E0E0'
                const bg = isSelected ? activeBg : '#FFFFFF'
                const color = isSelected ? activeColor : '#6B7280'
                const label = tag === 'good' ? 'Good Day' : 'Hard Day'
                const emoji = tag === 'good' ? '🌿 ' : '💙 '
                return (
                  <button
                    key={tag}
                    onClick={() => updateForm('day_tag', isSelected ? null : tag)}
                    style={{
                      flex: 1,
                      padding: '14px',
                      borderRadius: '14px',
            ...
            <div style={{ display: 'flex', gap: '12px' }}>
              <button style={backStyle} onClick={() => setStep(1)}>
                Back
              </button>
              <button
                style={{ ...btnStyle, flex: 1, opacity: submitting ? 0.7 : 1 }}
