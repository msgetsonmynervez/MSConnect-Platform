...
export default function Home() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#1C2B3A', display: 'flex',
      alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
      <div style={{ background: '#FAF7F2', borderRadius: '24px', padding: '40px 32px',
        width: '100%', maxWidth: '400px', textAlign: 'center' }}>
        <div style={{ fontFamily: 'Georgia, serif', fontSize: '36px', fontWeight: 600,
          color: '#1C2B3A', marginBottom: '8px' }}>
          MS<span style={{ color: '#5C7A6B' }}>Connect</span>
        </div>
        {user && (
          <>
            <p style={{ fontSize: '20px', margin: '20px 0 8px', color: '#1C2B3A' }}>
              Welcome, {user.display_name} 🌿</p>
            <p style={{ fontSize: '13px', color: '#6B7280', marginBottom: '24px' }}>
              @{user.username} · {user.ms_type?.toUpperCase()}</p>
          </>
        )}
        <div style={{ background: '#EDF3F0', borderRadius: '16px', padding: '20px', marginBottom: '24px' }}>
          <p style={{ fontSize: '14px', color: '#5C7A6B', fontWeight: 500 }}>
            🎉 Auth flow complete!</p>
          <p style={{ fontSize: '13px', color: '#6B7280', marginTop: '8px', lineHeight: 1.5 }}>
            Training, community, and progress screens coming next.</p>
        </div>
