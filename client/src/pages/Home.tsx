...
export default function Home() {
    ...
    async function load() {
      try {
        ...
        const [ci, str, tc, ts] = await Promise.all([
          ...
          supabase.from('training_streaks').select('current_streak_days,longest_streak_days,paused_since').eq('user_id', data.id).maybeSingle(),
  ...
  async function dismissAlert() {
    ...
    await supabase.from('users').update({
      alert_state: { ...alertState, dismissed_at: today }
    }).eq('id', user.id)
          ...
          {showSymptomAlert && (
            <div style={{
              ...
              <p style={{ fontSize: '13px', color: '#92400E', lineHeight: 1.6, marginBottom: '14px' }}>
                You've logged several difficult symptoms recently. Consider reaching out to your care team.
              </p>
              <button onClick={dismissAlert} disabled={dismissingAlert} style={{
                ...
                fontSize: '13px', color: '#92400E', cursor: 'pointer', fontWeight: 500
              ...
              {streakPaused ? (
                <div>
                  ...
                  <p style={{ fontSize: '13px', color: '#1D4ED8', lineHeight: 1.6 }}>
                    Your streak is paused while you rest. It will resume when you're feeling better.
                  </p>
                </div>
              ) : (
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    ...
                    <div style={{ fontSize: '28px', fontWeight: 700, color: '#1C2B3A' }}>
                      ...
                      <span style={{ fontSize: '14px', fontWeight: 400, color: '#6B7280' }}> days</span>
                    </div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ fontSize: '13px', color: '#6B7280', marginBottom: '4px' }}>Best</div>
                    <div style={{ fontSize: '20px', fontWeight: 600, color: '#5C7A6B' }}>
                      {streak.longest_streak_days}
                      <span style={{ fontSize: '12px', fontWeight: 400, color: '#6B7280' }}> days</span>
                    </div>
                  </div>
                </div>
              )}
            ...
            {checkedInToday ? (
              <div style={{ textAlign: 'center' }}>
                ...
                <div style={{ fontSize: '12px', color: '#6B7280', marginTop: '6px' }}>
                  Next in <span style={{ fontWeight: 600, color: '#5C7A6B' }}>{countdown}</span>
                </div>
              </div>
            ) : (
              <div>
            ...
            <div style={{ background: '#FAF7F2', borderRadius: '20px', padding: '20px' }}>
              ...
              {user.neurologist_phone && (
                <a href={'tel:' + user.neurologist_phone} style={{
                  fontSize: '13px', color: '#5C7A6B', fontWeight: 500, textDecoration: 'none'
                }}>
                  📞 {user.neurologist_phone}
                </a>
              )}
                  ...
                  return (
                    <button
                      ...
                      style={{
                        ...
                        fontSize: opt.size, fontWeight: sel ? 600 : 400,
